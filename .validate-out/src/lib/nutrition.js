...
exports.NUTRITION_FOOD_PRESETS = void 0;
exports.getNutritionValuesForGrams = getNutritionValuesForGrams;
exports.customNutritionFoodToPreset = customNutritionFoodToPreset;
exports.nutritionPresetToCustomFood = nutritionPresetToCustomFood;
exports.cleanNutritionNumber = cleanNutritionNumber;
exports.toNumber = toNumber;
exports.parseBirthDate = parseBirthDate;
exports.calcAge = calcAge;
exports.getNutritionPlan = getNutritionPlan;
exports.getNutritionTotals = getNutritionTotals;
exports.buildMacroMessage = buildMacroMessage;
exports.NUTRITION_FOOD_PRESETS = [
...
function getNutritionValuesForGrams(preset, gramsValue) {
...
function customNutritionFoodToPreset(food) {
    const baseQuantity = Math.max(1, food.baseQuantity || 100);
    return {
        id: food.id,
        name: food.name.trim(),
        baseAmount: food.baseMode === 'serving'
            ? 'per 1 serving'
...
function nutritionPresetToCustomFood(preset) {
    var _a, _b;
    ...
    return {
        id: preset.id,
        name: preset.name,
        brand: ((_a = preset.brand) === null || _a === void 0 ? void 0 : _a.trim()) || undefined,
        barcode: ((_b = preset.barcode) === null || _b === void 0 ? void 0 : _b.trim()) || undefined,
...
function cleanNutritionNumber(value) {
...
function getNutritionPlan({ dateOfBirth, heightCm, weightKg, goal, activityLevel, sex, calorieOverride, desiredWeightKg, pace, }) {
    ...
    const effectiveGoal = desiredWeight > 0
        ? desiredWeight < weight
            ...
            : desiredWeight > weight
                ...
                : 'maintain'
        : goal;
    const adjustment = effectiveGoal === 'gain'
        ? pace === 'fast'
            ? 320
            : 170
        : effectiveGoal === 'lose'
...
function buildMacroMessage(title, current, target, unit) {
    const diff = Math.round(target - current);
    if (diff > 0)
        return { title, text: `${diff} ${unit} left to hit your target.` };
    if (diff < 0)
        return { title, text: `${Math.abs(diff)} ${unit} over your target.` };
